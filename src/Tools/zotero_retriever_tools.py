...
def _assess_relevance(documents: List[Document], threshold: float) -> str:
    ...
    avg_distance = sum(doc.metadata.get('distance', 1.0) for doc in documents) / len(documents)
...
def semantic_search(
        query: str,
        runtime: ToolRuntime[config.RetrieverContext]
) -> List[Document]:
    """
...
def get_item_context(
    ...
    """
    Get document chunks from a specific item without semantic search.

    Args:
        item_id: The Zotero item ID to retrieve chunks from.
        runtime: Injected runtime context with vector storage and preferences.

    Returns:
        List of Document objects from the specified item.

    Use this to browse or get an overview of a specific document's content.
    """
    storage = runtime.context.vector_storage
    k = runtime.context.k_documents
...
def multi_query_search(
    ...
    results = all_results[:k * 2]  # Return up to 2x k_documents for comprehensive coverage
    result_string = _list_of_documents_to_string(results)
